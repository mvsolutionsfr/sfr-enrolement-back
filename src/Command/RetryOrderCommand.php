...
//
// php bin/console app:retrycommande  numCmd
#[AsCommand(
    name: 'app:retrycommande  numCmd',
    description: 'Cette commande rejour une commande en erreur',
    hidden: false
)]
class RetryOrderCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($commande != null) {
            ...
            $nbTransac = $transacs->count();
            /** @var OrderTransaction $transac */
            $transac = $transacs->get($nbTransac - 1);
            if ($transac && $transac->getStatut() == StatutEnrolementEnum::ErreurApplicative->value) {
                ...
                $transac = $transacs->get($nbTransac - 2);
                if ($transac && $transac->getStatut() == StatutEnrolementEnum::CheckStatut->value) {
                    $suivis = $transac->getTerminalSuivis();
                    foreach ($suivis as $suivi) {
                        $this->doctrine->getManager()->remove($suivi);
                    }
