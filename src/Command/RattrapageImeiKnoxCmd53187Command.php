...
//
// php bin/console app:rattrage_53187
#[AsCommand(
    name: 'app:rattrage_53187',
    description: 'Cette commande  fait un rattrapage des commandes sans vendorId',
    hidden: false
)]
class RattrapageImeiKnoxCmd53187Command extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        /** @var Orders $order */
        $orderTransaction = $this->doctrine->getRepository(OrderTransaction::class)->find(93536);
        $order = $this->doctrine->getRepository(Orders::class)->find(53187);
        $order = $orderTransaction->getOrders();
        $order->setDernierStatut(StatutEnrolementEnum::Ok->value);
        $terminal = $this->doctrine->getRepository(Terminal::class)->find(409413);
        $enrolement = new Enrolement();
        $enrolement->setTerminal($terminal);
$enrolement->setClient($order->getClient());
$enrolement->setDate($orderTransaction->getDateCreation());
        $this->doctrine->getManager()->persist($enrolement);

        $order->addTerminauxEnrole($terminal);
