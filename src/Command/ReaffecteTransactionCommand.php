...
class ReaffecteTransactionCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $transacs = array();
//        $transacs[] = [ 'transac' => '163557', 'order' => '75617'];
        $transacs[] = [ 'transac' => '163558', 'order' => '75617'];
//        $transacs[] = '51202';
//        $transacs[] = '51225';
//        $transacs[] = '51226';
//        $transacs[] = '51229';
...
//        $transacs[] = '51431';
        ...
        foreach ($transacs as $transac) {
            ...
            $transacId = $transac['transac'];
            $orderId =  $transac['order'];
            $output->writeln($transacId);
/** @var OrderTransaction $transac */
            /** @var Orders $orderDest */
            $transac = $this->doctrine->getRepository(OrderTransaction::class)->find($transacId);
            $orderDest = $this->doctrine->getRepository(Orders::class)->find($orderId);
            if ($transac != null) {
                $order = $transac->getOrders();
                $order->removeTransaction($transac);
                $orderDest->addTransaction($transac);
//                $orderDest->setDernierStatut(StatutEnrolementEnum::Ok->value);
//                foreach ($transac->getTerminalSuivis() as $suivi)  $orderDest->addTerminauxEnrole($suivi->getTerminal());
                $this->doctrine->getManager()->persist($order);
                $this->doctrine->getManager()->persist($orderDest);
